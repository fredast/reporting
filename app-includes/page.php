...
function page_header($page_title = APP_NAME, $additional_head = "") {
	...
	window.numeraljs = numeral.language("custom");</script>' . PHP_EOL;
	// Bootstrap
	$header .= '<link rel="stylesheet" href="' . LIB . '/bootstrap-3.3.2/css/bootstrap.min.css">' . PHP_EOL;
	$header .= '<script src="' . LIB . '/bootstrap-3.3.2/js/bootstrap.min.js"></script>' . PHP_EOL;
...
function navbar() {
	...
	$navbar .= '<div class="container-fluid">' . PHP_EOL;
	...
	$dropdown_content .= navbar_module_link('Meetings & Calls', array('report','meetings'));
	...
	if (isset($login)){
		$navbar .= '<p class="navbar-text">' . $login . '</p>' . PHP_EOL;
	}
    $navbar .= '</ul>' . PHP_EOL;
    $navbar .= '</div>' . PHP_EOL;
	$navbar .= '</div>' . PHP_EOL;
